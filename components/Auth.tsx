...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // loginSession(user, false); // No longer needed if verifyCredentials handles session, but registerUser just returns user.
            // Ensure session is set. registerUser maps user. 
            // Supabase auto-signs in on registration usually, unless email confirmation is on. 
            // Assuming auto-sign in:
            onLogin(user);
        } catch (err: any) {
                ...
                {mode === 'LOGIN' && (
                    <form onSubmit={handleLoginStart} className="space-y-4">
                        <div>
                            ...
                            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 rounded-xl border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:ring-2 focus:ring-brown-500 outline-none" placeholder="you@example.com" />
                        </div>
                        <div>
